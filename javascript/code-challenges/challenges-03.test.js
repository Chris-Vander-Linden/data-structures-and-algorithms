...
const notInFirstArray = (forbiddenValues, sourceValues) => {
  return sourceValues.filter(sourceValue => !forbiddenValues.includes(sourceValue));
};
...
const getBaseStatGreaterThan = (arrObjs, minBaseStat) => {
  return arrObjs.filter(arrObj => arrObj.baseStat > minBaseStat);
};
...
const getStatName = (arrObjs, minBaseStat) => {
  return arrObjs.filter(arrObj => arrObj.baseStat > minBaseStat).map(obj => obj.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  return arr.filter(elem => typeof elem === 'number').map(num => num % 2 ? 'odd' : 'even');
};
