...
NOTE: Although I could have manually created an array with unique measurements to
later use to extract the ingredients, which would have been much shorter, I wanted to try to program it, so I could practice some different methods.
...
const listFoods = ({ ingredients }) => {
  ...
  // get array of measurement 'flags' by getting second word
  const allMeasurements = ingredients.map(item => item.match(/\s(.*?)\s/)[1]);

  // get all unique measurements by checking to see if indexOf matches first instance of match.  If it doesn't, then don't return value.  Also, these need to be sorted by length, so plural measurements can be match first to prevent an error when later extracting ingredient.
  const uniqueMeasurements = allMeasurements.filter((item, index, array) => array.indexOf(item) === index).sort((a, b) => b.length - a.length);

  const result = [];
  ingredients.forEach(item => {
    // find first matching measurement for item
    ...
    result.push(item.slice(item.indexOf(match) + match.length).trim());
