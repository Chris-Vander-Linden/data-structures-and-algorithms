...
const countNumberOfChildren = (arr) => {
  ...
  return arr.reduce((accumulator, value) => {
    // I guess I can't use optional chaining in this challenge.
    return accumulator += value.children ? value.children.length : 0;
  }, 0);
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((accumulator, value) => {
    return accumulator += isPrime(value) ? 1 : 0;
  }, 0);
};
...
const extractChildren = (characters) => {
  ...
  return characters.filter(character => {
    ...
    // if no children, short circuit to next character
    character.children && character.children.forEach(child => {
