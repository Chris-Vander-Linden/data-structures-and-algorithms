...
const standardizePhoneNumbers = (arr) => {
  return arr.map(strNum => strNum.replace(/[()\s-]/g, ''));
};
...
const unenrollBrook = (arr) => {
  ...
  return arr.map(innerArr => innerArr.filter(str => !str.includes('Brook')));
...
const sortByDay = (arr) => {
  ...
  const resultArr = new Array(7).fill([]);

  for (let idx = 0; idx < daysOfWeek.length - 1; idx++) {

    const dayOfWeekArr = [];

    for (const element of arr) {
      element.includes(daysOfWeek[idx]) && dayOfWeekArr.push(element);
    }

    resultArr[idx] = dayOfWeekArr;
