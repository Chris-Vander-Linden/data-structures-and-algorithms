...
const updateAnimal = (arr, callback) => {
  return arr.map(elem => {
    return callback === upper ? upper(elem) : lower(elem);
  });
};
...
const sortPeopleBetter = (arr) => {
  ...
  return arr.sort((a, b) => {
    return a.lastName < b.lastName ? -1 : a.lastName > b.lastName ? 1 : a.firstName < b.firstName ? -1 : a.firstName > b.firstName ? 1 : a.age < b.age ? -1 : a.age > b.age ? 1 : 0;
  });
...
const sortMeetingsByDay = (arr) => {
  ...
  const meetingsWithKeys = arr.map(meeting => {
    let dayKey = null;
    ...
    return { dayKey, meeting };
  });

  return meetingsWithKeys.sort((a, b) => {
    return a.dayKey - b.dayKey;
  }).map(meetingObj => meetingObj.meeting);
};
...
const sortSchedule = (arr) => {
  ...
  return sortMeetingsByDay(arr).sort((a, b) => {
    return a.dayOfWeek !== b.dayOfWeek ? 0 : a.start < b.start ? - 1 : a.start > b.start ? 1 : (a.end - a.start) < (b.end - b.start) ? -1 : (a.end - a.start) > (b.end - b.start) ? 1 : 0;
  });
};
